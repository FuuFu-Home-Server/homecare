...
const STEPS = ["Identitas", "Kontak & Jaminan", "Riwayat & Gaya Hidup"] as const;
      ...
      {STEPS.map((label, i) => {
        ...
        return (
          <div key={label} className="flex flex-1 items-center gap-2">
            ...
            {i < STEPS.length - 1 ? (
              <span className={cn("h-px flex-1", done ? "bg-brand-300" : "bg-slate-200")} />
            ) : null}
          </div>
        );
      })}
...
export function PatientRegisterModal({
  ...
  // Restart at the first step each time the modal opens.
  ...
  return (
    ...
    >
      <Stepper current={step} onStep={(s) => (s < step ? setStep(s) : canNext ? setStep(s) : null)} />
      ...
      {step === 1 ? (
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <Textarea label="Alamat" rows={2} value={form.alamat} onChange={(e) => set("alamat", e.target.value)} />
          </div>
          <div>
            <Label>Agama</Label>
            <CustomSelect
